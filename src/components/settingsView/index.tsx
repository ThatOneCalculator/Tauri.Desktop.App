...
const Settings = () => {
	...
	const [servers, setServers] = useState<RCServer[]>([
		{ serverUrl: "https://open.rocket.chat", isDefault: true },
	]);
	const [defaultServerIndex, setDefaultServerIndex] = useState<number>(0);
	const [serverUrl, setServerUrl] = useState<string>("");
	const [notificationsAllowedState, setNotificationsAllowed] =
		useState<boolean>(true);
	const [newWindowsOnServerSwitchState, setNewWindowsOnServerSwitch] =
	...
	useEffect(() => {
		store.load().then(async () => {
			...
			if (servers) {
				...
				const defaultServer = (await store.get("defaultServer")) as {
					serverUrl: string;
					index: number;
				};
			...
			const newWindowsOnServerSwitch = await store.get(
				"newWindowsOnServerSwitch",
			);
	...
	useEffect(() => {
		store
			.set("defaultServer", {
				serverUrl: servers[defaultServerIndex],
				index: defaultServerIndex,
			})
			.then(() => {
				store.save().then(() => {
					console.log("Servers Saved");
	...
	useEffect(() => {
		store
			.set("newWindowsOnServerSwitch", newWindowsOnServerSwitchState)
			.then(() => {
				store.save().then(() => {
					console.log("Servers Saved");
				});
	...
	const makeDefaultServer = (index: number, serverUrl: string) => {
		...
		store
			.set("defaultServer", { serverUrl: serverUrl, index: index })
			.then(() => {
				store.save().then(() => {
					invoke("change_default_server", { defaultServer: serverUrl });
	...
	const addServer = () => {
		setServers((prevServers) => [
			...prevServers,
			{ serverUrl: serverUrl, isDefault: false },
		]);
	};
				...
				<div className="generalSettings">
					<div className="checkboxItem">
						<input
							...
							type="checkbox"
							defaultChecked={notificationsAllowedState}
							value={"checked"}
					...
					<div className="checkboxItem">
						<input
							...
							type="checkbox"
							name="Allow Notifications"
							defaultChecked={newWindowsOnServerSwitchState}
						/>
				...
				<div className="serversPane">
					<div className="serverAddPane">
						<input
							...
							placeholder="Enter Rocket.Chat Server URL"
						/>
						<button onClick={() => addServer()} className="serverAddButton">
							Add
						</button>
					</div>
					<ServerList
